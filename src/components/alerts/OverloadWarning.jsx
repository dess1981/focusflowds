...
import { AlertTriangle, ChevronDown, ChevronUp, ArrowRight, Loader2 } from 'lucide-react';
...
export default function OverloadWarning() {
  ...
  useEffect(() => {
    checkOverload();
    const interval = setInterval(checkOverload, 60000); // Check every minute
  ...
  if (loading || !analysis || analysis.status === 'balanced') {
    return null;
  }
                ...
                <div className="space-y-1.5">
                  {analysis.overloadedDays.map(day => (
                    <div key={day.date} className="flex items-center justify-between text-sm p-2 rounded bg-destructive/10">
                      <span>{format(new Date(day.date), 'EEEE, d MMM', { locale: ptBR })}</span>
                  ...
                  {analysis.suggestions.map((suggestion, idx) => (
                    <div key={idx} className="p-2.5 rounded bg-muted/40 border border-primary/20 text-xs">
                      ...
                      <div className="space-y-1">
                        {suggestion.tasksToMove.map(task => (
